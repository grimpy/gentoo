...
SRC_URI="${REL_URI}/${MY_PV}/linux-x86_64/en-US/${MY_P}.tar.bz2
...
for X in ${LANGS} ; do
	if [ "${X}" != "en" ] && [ "${X}" != "en-US" ]; then
		SRC_URI="${SRC_URI}
			linguas_${X/-/_}? ( ${REL_URI}/${MY_PV}/linux-x86_64/xpi/${X}.xpi -> ${P/-bin/}-${X}.xpi )"
	fi
	...
	if [ "${#X}" == 5 ] && ! has ${X} ${NOSHORTLANGS}; then
		if [ "${X}" != "en-US" ]; then
			SRC_URI="${SRC_URI}
				linguas_${X%%-*}? ( ${REL_URI}/${MY_PV}/linux-x86_64/xpi/${X}.xpi -> ${P/-bin/}-${X}.xpi )"
		fi
